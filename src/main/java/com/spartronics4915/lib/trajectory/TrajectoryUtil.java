...
public class TrajectoryUtil
{
    ...
    public static <S extends IPose2<S>> Trajectory<S> mirror(final Trajectory<S> trajectory)
    {
    ...
    public static <S extends IPose2<S>> Trajectory<TimedState<S>> mirrorTimed(final Trajectory<TimedState<S>> trajectory)
    {
    ...
    public static <S extends IPose2<S>> Trajectory<S> transform(final Trajectory<S> trajectory, Pose2 transform)
    {
        List<S> waypoints = new ArrayList<>(trajectory.length());
        for (int i = 0; i < trajectory.length(); ++i)
        {
    ...
    public static Trajectory<Pose2WithCurvature> trajectoryFromPathFollower(IPathFollower path_follower,
            Pose2WithCurvature start_state, double step_size, double dcurvature_limit)
    {
        List<Pose2WithCurvature> samples = new ArrayList<Pose2WithCurvature>();
        samples.add(start_state);
        Pose2WithCurvature current_state = start_state;
        while (!path_follower.isDone())
        {
            // Get the desired steering command.
            final Twist2 raw_steering_command = path_follower.steer(current_state.getPose());

            // Truncate to the step size.
            Twist2 steering_command = raw_steering_command;
            ...
            {
                steering_command = new Twist2(steering_command.dx, steering_command.dy,
            ...
            final Twist2 average_steering_command = !curvature_valid
                    ? steering_command
                    : new Twist2(steering_command.dx, steering_command.dy,
                            (current_state.getCurvature() + 0.5 * dcurvature * steering_command.norm())
                                    * steering_command.norm());
            current_state = new Pose2WithCurvature(
                    current_state.getPose().transformBy(Pose2.exp(average_steering_command)),
        ...
        return new Trajectory<Pose2WithCurvature>(samples);
    }

    public static Trajectory<Pose2WithCurvature> trajectoryFromSplineWaypoints(final List<Pose2> waypoints, double maxDx, double maxDy,
            double maxDTheta)
    {
    ...
    public static Trajectory<Pose2WithCurvature> trajectoryFromSplines(final List<? extends Spline> splines, double maxDx,
